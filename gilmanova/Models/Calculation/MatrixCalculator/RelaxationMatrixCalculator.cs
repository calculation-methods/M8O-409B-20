...
namespace Numerics7
{
    internal class RelaxationMatrixCalculator : MatrixCalculator
    {
        private const double lowestOmega = 0.0;
        private const double highestOmega = 2.0;
        private double omega;

        public RelaxationMatrixCalculator(Issue issue): base(issue) { }
        ...
        public double Omega
        {
            ...
            set
            {
                if (value > lowestOmega - 0.00001 && value < highestOmega + 0.00001)
                {
                    omega = value;
                }
                else
        ...
        protected override Matrix CalculateIteration(Matrix prevMatrix)
        {
            ...
            for (int j = 0; j < prevMatrix.Count; j++)
            {
                ...
                {
                    nextMatrix[j][i] = (1 - Omega) * prevMatrix[j][i] + Omega * nextMatrix[j][i];
                }
            }
