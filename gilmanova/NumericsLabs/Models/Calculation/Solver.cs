...
namespace Numerics5
{
    internal class Solver
    {
        ...
        public Matrix SolveWith(CalculationMethod calculationMethod)
        {
            IStepCalculator stepCalculator;
            switch (calculationMethod)
            {
                case CalculationMethod.CLEAR:
                    stepCalculator = new ClearStepCalculator(Issue);
                    break;
                case CalculationMethod.UNCLEAR:
                    stepCalculator = new UnclearStepCalculator(Issue);
                    break;
                case CalculationMethod.CRANK_NIKOLSON:
                    stepCalculator = new CrankNicolsonStepCalculator(Issue);
                    break;
                default:
