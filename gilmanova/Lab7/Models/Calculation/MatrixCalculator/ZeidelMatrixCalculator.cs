...
namespace Numerics7
{
    internal class ZeidelMatrixCalculator : MatrixCalculator
    {
        public ZeidelMatrixCalculator(Issue issue): base(issue) { }
        ...
        protected override Matrix CalculateIteration(Matrix prevMatrix)
        {
            ...
            for (int j = 0; j < prevMatrix.Count; j++)
            {
                ...
                for (int i = 0; i < prevMatrix[j].Count; i++)
                {
                    nextMatrix[j][i] = prevMatrix[j][i];
                }
            }

            for (int j = 1; j < nextMatrix.Count - 1; j++)
            {
                for (int i = 1; i < nextMatrix[j].Count - 1; i++)
                {
                    ...
                    var usum = leftKoef * nextMatrix[j][i - 1] + rightKoef * nextMatrix[j][i + 1] + topKoef * nextMatrix[j - 1][i] + bottomKoef * nextMatrix[j + 1][i];
