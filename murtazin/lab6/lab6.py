...
def explicit(K, t, tau, h, x, approx_st, approx_bo):
    ...
    for j in range(N):
        ...
        if approx_st == 2:
            U[1][j] = np.sin(x[j]) - np.sin(x[j]) * tau + (
                        -2 * np.sin(x[j]) + np.cos(x[j]) + func(x[j], t)) * tau ** 2 / 2

    for k in range(1, K - 1):
        t += tau
        for j in range(1, N - 1):
            U[k + 1, j] = ((2 + 3 * tau) * U[k, j] - U[k - 1, j] + \
                           tau ** 2 * ((U[k, j + 1] - 2 * U[k, j] + U[k, j - 1]) / h ** 2 + (
                                U[k, j + 1] - U[k, j - 1]) / (2 * h) - U[k, j] - func(x[j], t))) \
                          / (1 + 3 * tau)
        if approx_bo == 1:
            ...
            U[k + 1, 0] = (2 * tau ** 2 * (h - h ** 2 / 2) * func_border1(t) - 2 * tau ** 2 * U[
                k + 1, 1] + tau ** 2 * h ** 2 * func(x[0], t) \
                           - h ** 2 * (2 + 3 * tau) * U[k, 0] + h ** 2 * U[k - 1, 0]) / (
                                      -2 * tau ** 2 - tau ** 2 * h ** 2 - h ** 2 - 3 * h ** 2 * tau)
            U[k + 1, N - 1] = (2 * tau ** 2 * (h + h ** 2 / 2) * func_border2(t) + 2 * tau ** 2 * U[
                k + 1, N - 2] - tau ** 2 * h ** 2 * func(x[N - 1], t)
                               + h ** 2 * (2 + 3 * tau) * U[k, N - 1] - h ** 2 * U[k - 1, N - 1]) / (
                                          2 * tau ** 2 + tau ** 2 * h ** 2 + h ** 2 + 3 * h ** 2 * tau)
...
def implicit(K, t, tau, h, x, approx_st, approx_bo):
    ...
    for k in range(1, K - 1):
        ...
        if approx_bo == 1:
            ...
            d[N - 1] = func_border2(t) * (h + h ** 2 / 2) - (func(x[N - 1], t) * h ** 2) / 2 + (
                        U[k, N - 1] * h ** 2) / tau ** 2 \
